...
class GameViewModel(
    ...
    suspend fun validateCurrentPuzzle() {
        ...
        val incorrectCells = computeIncorrectCells(currentPuzzle.cells, puzzle.solution)
        _uiState.value = _uiState.value.copy(
            isCompleted = isValid,
    ...
    private fun applyMove(row: Int, col: Int, value: Int) {
        ...
        val solution = _uiState.value.puzzle?.solution
        val incorrectCells = if (solution == null) {
            emptySet()
        } else {
            computeIncorrectCells(updatedCells, solution)
        }
        val isCompleted = updatedCells.none { it.value == 0 } && incorrectCells.isEmpty()
        _uiState.value = _uiState.value.copy(
            cells = updatedCells,
...
data class GameUiState(
    ...
    val incorrectCells: Set<CellPosition> = emptySet(),
...
private fun computeIncorrectCells(
    cells: List<com.sudoku.app.domain.model.Cell>,
    solution: IntArray
): Set<CellPosition> {
    return cells.filter { cell ->
        val index = cell.row * 9 + cell.col
